...
logging.basicConfig()
moduleLogger = logging.getLogger('c2w.protocol.tcp_chat_client_protocol')  ##name of the log
...
#    
    ...
    def timeout(self,data,functionid):
        ...
        if(self.flag==0):
            ###### set function id for type of message request
            
            if(functionid==0):
                self.obj=reactor.callLater(1,self.sendLoginRequestOIE,data)
            if(functionid==1):
                self.obj=reactor.callLater(1,self.sendJoinRoomRequestOIE,data)
            if(functionid==2):
                self.obj=reactor.callLater(1,self.sendChatMessageOIE,data)
	    if(functionid==3):
                self.obj=reactor.callLater(3,self.sendLeaveSystemRequestOIE)
            
    ### function for process the framing algrothirm       
    def bufferHandler(self,data):
        buf=self.buf+data
	...
	flag=0
        ...
        if len(buf)>=6:
            ...
            data_length=struct.unpack_from('!H',header[4:6])[0]
           
            if data_length <=len(buf)-6:
               
                packet=buf[0:6+data_length]
		
                self.buf=buf[6+data_length:]
		
		if len(self.buf)>=6:                          #if flag ==1, means in self.buf still exist a complet packet 
		    
		    data_length2=struct.unpack_from('!H',self.buf[4:6])[0]
		    
		    if data_length2 <=len(self.buf)-6:			
			flag=1
                return packet,flag
            else:
    ...
    def dataReceived(self, data):
        ...
        connection.
        """
        #### check condition for use framing algrothirm
	pass
        
        x=data
        flag=1
        while(flag==1):
            buf,flag=self.bufferHandler(x)
            x=''
            if(buf==None):
                return    
        
        
            tuple1=struct.unpack_from('!B',buf[0])[0]
            tuple2=struct.unpack_from('!B',buf[1])[0]
            tuple3=struct.unpack_from('!B',buf[2])[0]
            tuple4=struct.unpack_from('!B',buf[3])[0]
	    tuple5=struct.unpack_from('!H',buf[4:6])[0]
	    tuple6=struct.unpack_from(str(tuple5)+'s',buf[6:])[0]
	    
    
	    frg=self.getbit(tuple1,1,1)
	
	    
	    ack=self.getbit(tuple1,2,2)
	    mestype=self.getbit(tuple1,3,6)
	    
	    roomtype=self.getbit(tuple1,7,8)
	
	    
	    
	    serversequencenb=tuple2	
	    ...
	    if(ack==0 and mestype==12): ### chat message 
		...
		#### check condition for fragmentation
		...
		if(frg==0 and self.prefrg==0):
		    ...
		    self.flagmovie=1              ### flag indicate that client has already received the movie list
		...
		while(i<self.userlistlength):                
		    ...
		    p=i
		    i=i+3+namelength
		    usernameoflist=struct.unpack_from(str(i-p-3)+'s',self.userlist[p+3:i])[0]
		...
		if(self.location==-1):    #### location=-1 for the first time client receive movie list after login sucessful
		    ...
		    ################u for  and display movie and user list
		...
		if(ack==1 and mestype==8 and self.location==0):            
		    self.nbtime=0
		    self.flag=1
		    #self.timeout(data)
    ...
    #### function for get bit from byte
